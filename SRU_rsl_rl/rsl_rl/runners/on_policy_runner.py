...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        obs, critic_obs = self._extract_policy_and_critic_obs(obs_data)
        num_obs = obs.shape[-1]
        num_critic_obs = critic_obs.shape[-1] if critic_obs is not None else num_obs
        ...
        if self.is_mdpo:
            ...
            actor_critic_1: ActorCritic | ActorCriticRecurrent | ActorCriticSRU = (
                actor_critic_class(
                    num_obs, num_critic_obs, self.env.num_actions, **self.policy_cfg
                ).to(self.device)
            )
            actor_critic_2: ActorCritic | ActorCriticRecurrent | ActorCriticSRU = (
                actor_critic_class(
                    num_obs, num_critic_obs, self.env.num_actions, **self.policy_cfg
                ).to(self.device)
            )
            self.alg = alg_class(
                actor_critic_1, actor_critic_2, device=self.device, **self.alg_cfg
            )
        else:
            # Standard algorithms use one actor-critic
            actor_critic: ActorCritic | ActorCriticRecurrent | ActorCriticSRU = (
                actor_critic_class(
                    num_obs, num_critic_obs, self.env.num_actions, **self.policy_cfg
                ).to(self.device)
            )
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
        ...
        for it in range(start_iter, tot_iter):
            ...
            if self.video_recorder:
                ...
                if it % 100 == 0:
                    print(
                        f"[DEBUG VideoRecorder] Iteration {it}, checking should_record..."
                    )
            ...
            with torch.no_grad():
                for i in range(self.num_steps_per_env):
                    ...
                    policy_obs, critic_obs_raw = self._extract_policy_and_critic_obs(
                        obs_data, infos
                    )
                    obs = self.obs_normalizer(policy_obs)
                    if critic_obs_raw is not None:
            ...
            if self.log_dir is not None:
                ...
                if (
                    ...
                    and self.video_recorder.is_complete()
                ):
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if self.is_mdpo and locs["mean_kl_divergence"] is not None:
            self.writer.add_scalar(
                "Loss/kl_divergence", locs["mean_kl_divergence"], locs["it"]
            )
    ...
    def load(self, path, load_optimizer=True):
        ...
        if self.is_mdpo:
            self.alg.actor_critic_1.load_state_dict(
            ...
            if load_optimizer:
                self.alg.optimizer_1.load_state_dict(
    ...
    def get_inference_policy(self, device=None):
        ...
        if self.cfg["empirical_normalization"]:
            ...
            policy = lambda x: actor_critic.act_inference(
    ...
    def _extract_policy_and_critic_obs(self, obs_data, infos: dict | None = None):
        """Extract policy and critic observations from both old and new env APIs."""
        # New API: TensorDict or dict with explicit keys
        if isinstance(obs_data, Mapping) or hasattr(obs_data, "keys"):
            policy_obs = None
            critic_obs = None

            if "policy" in obs_data:
                policy_obs = obs_data["policy"]
            elif "obs" in obs_data:
                policy_obs = obs_data["obs"]

            if "critic" in obs_data:
                critic_obs = obs_data["critic"]

            # Last-resort fallback: use the first available observation entry.
            if policy_obs is None:
                for key in obs_data.keys():
                    policy_obs = obs_data[key]
                    break

            if policy_obs is None:
                raise RuntimeError(
                    "Could not extract policy observations from environment output."
                )

            if critic_obs is None and infos is not None:
                critic_obs = infos.get("observations", {}).get("critic", None)

            return policy_obs, critic_obs

        # Old API compatibility: tuple (obs, extras)
        if isinstance(obs_data, tuple) and len(obs_data) == 2:
            obs, extras = obs_data
            critic_obs = extras.get("observations", {}).get("critic", None)
            return obs, critic_obs

        # Legacy tensor-only API
        if torch.is_tensor(obs_data):
            critic_obs = None
            if infos is not None:
                critic_obs = infos.get("observations", {}).get("critic", None)
            return obs_data, critic_obs

        raise TypeError(
            f"Unsupported observation type from environment: {type(obs_data)}"
        )
